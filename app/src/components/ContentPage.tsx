...
export default function ContentPage({ onNext }: ContentPageProps) {
  ...
  return (
    <div className="min-h-screen relative overflow-hidden px-4 py-8 md:py-12">
      ...
      <div className={`max-w-5xl mx-auto relative z-10 transition-all duration-700 ${showContent ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
        {/* Header */}
        <div className="text-center mb-10">
          <Heart size={28} className="mx-auto text-pink-500 fill-pink-400 mb-3 pulse-soft" />
          <h1 className="text-3xl md:text-4xl font-bold text-gray-800 mb-3 shimmer-text" style={{ fontFamily: "'Playfair Display', serif" }}>
            Our Moments
          </h1>
          <p className="text-gray-500 text-sm md:text-base max-w-md mx-auto">Ini adalah rangkuman dari beberapa momen yang kita nikmati berdua. Ada apa aja tuchhhh</p>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 mb-10">
          {contentItems.map((item, index) => (
            ...
            >
              ...
              <div className="p-4">
                <h3 className="font-bold text-gray-800 mb-1 group-hover:text-pink-600 transition-colors" style={{ fontFamily: "'Playfair Display', serif" }}>
                  {item.title}
                </h3>
      ...
      {/* Detail Modal */}
      {selectedItem && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 modal-backdrop" style={{ backgroundColor: 'rgba(0, 0, 0, 0.6)' }} onClick={() => setSelectedItem(null)}>
          <div className="bg-white rounded-3xl shadow-2xl max-w-lg w-full overflow-hidden animate-in fade-in zoom-in-95 duration-300" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="p-6 md:p-8">
              ...
              <p className="text-gray-600 leading-relaxed text-sm md:text-base">{selectedItem.fullDescription}</p>
            </div>

            {/* Navigation */}
            <div className="px-6 pb-6 flex justify-between items-center">
              <button onClick={handlePrev} className="flex items-center gap-1 text-pink-500 hover:text-pink-700 transition-colors text-sm font-medium">
                <ChevronLeft size={18} />
                Sebelumnya
              </button>
              ...
              <button onClick={handleNext} className="flex items-center gap-1 text-pink-500 hover:text-pink-700 transition-colors text-sm font-medium">
                Selanjutnya
