...
export default function AlbumPage({ onNext }: AlbumPageProps) {
  ...
  const filteredPhotos = activeCategory === 'Semua' ? albumPhotos : albumPhotos.filter((photo) => photo.category === activeCategory);
    ...
    <div className="min-h-screen relative overflow-hidden px-4 py-8 md:py-12">
      ...
      <div className={`max-w-5xl mx-auto relative z-10 transition-all duration-700 ${showContent ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-gray-500 text-sm md:text-base max-w-md mx-auto">Koleksi Foto mu ada 1000 lebih jir gak bisa upload semua wak.</p>
        </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 modal-backdrop" style={{ backgroundColor: 'rgba(0, 0, 0, 0.75)' }} onClick={() => setSelectedPhoto(null)}>
          <div className="bg-white rounded-3xl shadow-2xl max-w-md w-full overflow-hidden animate-in fade-in zoom-in-95 duration-300" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="p-5 text-center">
              ...
              <span className="inline-block px-3 py-1 bg-pink-100 text-pink-600 text-xs rounded-full">{selectedPhoto.category}</span>
            </div>
          </div>
        </div>
      )}
